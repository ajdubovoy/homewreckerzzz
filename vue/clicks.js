...
function init() {
    ...
    masterDry.connect(context.destination);
    masterDry.gain.value = 0.000001;
    var nextNoteTime = 50.0 + delay;
    var scheduleAheadTime = 100.0;
    //var interval = Math.random() * 3 * 100 + 200;
    var interval = isMobile ? 375.0 : 625.0;
    setInterval(function() {
        while (nextNoteTime < context.currentTime*1000 + scheduleAheadTime ) {
            scheduleNote(nextNoteTime/1000);
            nextNoteTime += interval;
            console.log("click " + nextNoteTime);
        }
    }, 50);
}
$("#start").on("click", function() {
    masterDry.gain.exponentialRampToValueAtTime(0.9, context.currentTime + 1);
});
$("#stop").on("click", function() {
    //ramp down the gain (fade out)
    masterDry.gain.linearRampToValueAtTime(0.0000000001, context.currentTime + 1);
});
function scheduleNote(time) {
    var osc = context.createOscillator();
    var gain = context.createGain();
    osc.connect(gain);
    gain.connect(masterDry);
    osc.frequency.setValueAtTime(isMobile ? 2000 : 1000, context.currentTime);
    osc.start(time);
